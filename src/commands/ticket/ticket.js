...
const { isTicketChannel, closeTicket, closeAllTickets, handleTicketOpen, handlePrivateTicket } = require("@handlers/ticket");
...
module.exports = {
  ...
  category: "TICKET",
  requiredRoleId: "1268995995375505458", //mod+ role
  command: {
    enabled: true,
    minArgsCount: 1,
    subcommands: [
  ...
  slashCommand: {
    ...
    options: [
      ...
      {
        name: "create",
        description: "create a private ticket with someone ",
        type: ApplicationCommandOptionType.Subcommand,
        options: [
          {
            name: "user",
            description: "the user to create the ticket with",
            type: ApplicationCommandOptionType.User,
            required: true,
          },
        ],
  ...
  async interactionRun(interaction, data) {
    ...
    // Remove from ticket
      ...
      const user = interaction.options.getUser("user")
      await handlePrivateTicket(interaction, user)
    }
    else if (sub == "rename") { 
      const allowedRoles = ["1267958988126027848", "1267959002164232292", "1267959000830578792"]; // Admins, Mods, Supervisors
      ...
      const newBaseName = interaction.options.getString("new-name")?.trim();
      const channel = interaction.channel;

      if (!newBaseName) {
        return interaction.followUp({ content: "Please provide a valid new name.", ephemeral: true });
      }

      // Simple check to see if it's a ticket channel, adjust based on your system
       if (!isTicketChannel(channel)) return "This command can only be used in ticket channel";

      const match = channel.name.match(/(\d+)$/);
      if (!match) {
        return interaction.followUp({ content: "Could not extract ticket number from the current channel name.", ephemeral: true });
      }

      const ticketNumber = match[1];
      const sanitizedBase = newBaseName
        .toLowerCase()
        .replace(/[^a-z0-9\-]/g, "-")
        .replace(/-+$/, "");

      const finalName = `${sanitizedBase}-${ticketNumber}`;

      if (!channel.manageable) {
        return interaction.followUp({ content: "I don't have permission to rename this channel.", ephemeral: true });
      }

      try {
        await channel.setName(finalName);
        return interaction.followUp({ content: `Channel renamed to **${finalName}**.` });
      } catch (err) {
        console.error("Rename failed:", err);
        return interaction.followUp({ content: "An error occurred while renaming the channel.", ephemeral: true });
      }
    }
