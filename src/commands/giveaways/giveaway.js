...
async function runModalSetup({ member, channel, guild }, targetCh) {
    ...
    new ModalBuilder({
      ...
      components: [
        ...
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            .setCustomId("host")
            .setLabel("User hosting the giveaway (id, username)")
            .setStyle(TextInputStyle.Short)
            .setRequired(false)
        ),
      ],
  ...
  if (/^\d+$/.test(host)) {
      // Check if host is a valid user ID
      const user = guild.client.users.cache.get(host);
      if (user) {
          hostId = host;
      } else {
          return modal.editReply("Setup has been cancelled. You need to provide a valid id or username for host");
      }
  } else {
      // Check if host is a username
      const user = guild.client.users.cache.find(u => u.username === host);
      if (user) {
          hostId = user.id;
      } else {
          return modal.editReply("Setup has been cancelled. You need to provide a valid id or username for host");
  ...
  const finalHost = guild.client.users.cache.get(hostId)
 
    


  const response = await start(member, targetCh, duration, prize, winners, finalHost, allowedRoles);
