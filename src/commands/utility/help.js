 ...
 */
async function getHelpMenu({ client, guild, channel }) {
  // Menu Row
  const options = [];
  for (const [k, v] of Object.entries(CommandCategory)) {
    if (v.enabled === false) continue;
    if (channel.nsfw === false && v.name.toLowerCase() === "nsfw") continue;
    options.push({
      label: v.name,
      value: k,
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "**About Me:**\n" +
        `Hello I am <@${client.user.id}>!\n` +
        "I am a multipurpose Discord bot. Check out my commands!" 
    );
