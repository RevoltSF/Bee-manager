...
const buildGreeting = async (member, type, config, inviterData) => {
  ...
  if (config.embed.footer || config.embed.footer_image) {
    const footerText = config.embed.footer ? await parse(config.embed.footer, member, inviterData) : null;
    const footerIconURL = config.embed.footer_image ? await parse(config.embed.footer_image, member, inviterData) : null;
  ...
  if(config.embed.author_content || config.embed.author_image){
    const authorName = config.embed.author_content ? await parse(config.embed.author_content, member, inviterData) : null;
    const authorIconURL = config.embed.author_image ? await parse(config.embed.author_image, member, inviterData) : null;
