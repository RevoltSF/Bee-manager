...
const { TICKET, EMBED_COLORS } = require("@root/config.js");
...
async function handleTicketOpen(interaction) {
  ...
  try {
    const idRole = interaction.guild.roles.cache.get(require("@root/config").TICKETING.modrole)
    const ee =interaction.client.ticketdatabase.get("ticket") || 0
    const ticketNumber = Number(ee) + 1;
    if(ticketNumber == 1){
      interaction.client.ticketdatabase.set("ticket", ticketNumber);
    }else{
      interaction.client.ticketdatabase.delete("ticket");
      interaction.client.ticketdatabase.set("ticket", ticketNumber);
    }
    ...
    const categoryChannel = guild.channels.cache.get(require("@root/config").TICKETING.category)
    ...
    const sent = await tktChannel.send({ content: `${user.toString()} <@&${require("@root/config").TICKETING.modrole}>`, embeds: [embed], components: [buttonsRow] });
 ...
 */
async function handlePrivateTicket(interaction, target) {
